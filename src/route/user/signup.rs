...
use crate::{types::{ErrorResponse, SignUpRequest, SignUpResponse}, utility::get_psql_pool};

pub async fn signup(pool: web::Data<Pool>,data: web::Json<SignUpRequest>) -> std::io::Result<impl Responder>{
    ...
    match psql_client
        .query_one(
            query,
            &[&user_id.to_string(), &data.username, &password_hash],
        )
        .await
    {
        Ok(row) => {
            let returned_user_id: String = row.get::<_,String>("user_id");
            let returned_username: String = row.get::<_,String>("username");
        ...
        Err(e) => {
            match psql_client.query_one(
                "SELECT EXISTS(SELECT * FROM \"user\" WHERE username = $1)",
                &[&data.username]
            ).await
            {
                Ok(is_exist)=>{
